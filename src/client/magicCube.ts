...
export class MagicCube {
    ...
    private bShowCubeAxes: boolean = false;
    private midPointArr: Array<AxesHelper>;


    //是否显示面中心方块的局部坐标系
    private isShowMidAxis: Array<boolean> = new Array(
        // Right,Left,
        true, false,
        // Up,Down,
        false, false,
    ...
    private setIsShowMidAxis(isShow: boolean) {
    ...
    private initMidPointArr() {
        this.midPointArr = new Array<AxesHelper>(6);
    ...
    private getMidAxes(direction: cubeDirection): AxesHelper {
     ...
     * @param obj 
     */
    private getWorldPosition(obj: THREE.Object3D, isNeedRound = false): Vector3 {
     ...
     * @param obj 
     */
    private makeWorldPositionRound(obj: THREE.Mesh) {
        console.log();
        let worldPositon = new THREE.Vector3();
        obj.getWorldPosition(worldPositon);
     ...
     * @param direction 
     */
    public updateRelativePos(direction: cubeDirection) {
    ...
    private makeRotationX(position, angle): Matrix4 {
        var q = new Quaternion();
        q.setFromAxisAngle(new THREE.Vector3(1, 0, 0), angle);
        var matrix: Matrix4 = new THREE.Matrix4().compose(position, q, new Vector3(1, 1, 1));
        return matrix;
    }
